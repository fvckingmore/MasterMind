...
void juego(){ //ENTRADA DE COLORES DEL USUARIO
	...
	for(i=0;i<=2;i++){
		...
		do{
			...
			printf("Color #%d => ",i+1); scanf("%c",&l); buf();
			...
			switch(l){
				...
				default: printf("Opcion incorrecta, vuelva a intentar\n"); 
						 getchar(); B=1; /*system("clear");*/ break;
			}
...
void cont(int *op){ /*SE ENCARGA DE IMPRIMIR LA OPCION AL JUGADOR SI QUIERE CONTINUAR O SALIR*/
	...
	do{
		...
		printf("\n¿Desea continuar?\n\n \tSi => S \t|| \tNo => N\n\nOpcion => "); scanf("%c",&opcion); buf();

		switch(opcion){
			
			case 's':
			case 'S': *op=1; printf(barras); printf("\n"); intentos++; /*system("clear");*/ break;
			
			case 'n':
			case 'N': *op=0; system("clear"); printf(barras); printf("\n"); system("clear"); break;
			
			default: printf("Opcion incorrecta, vuelva a intentar"); *op=2; getchar(); system("clear"); break;
		}
	}while(*op==2);
